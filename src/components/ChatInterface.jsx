...
console.log(theme,'theme')
  return (
    ...
    >
      {message.type === "user" ? (
        <Typography variant="body1" sx={{ color: theme.palette.text.primary==="#ffffff"?"#000000":"#ffffff"}}>
          {message.content}
        </Typography>
      ) : message.content ? (
...
const ChatInput = ({
  ...
  >
    <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
      <IconButton size="small" onClick={handleFileClick} disabled={isUploading}>
        {isUploading ? (
...
const ChatInterface = () => {
  ...
  const handleFileChange = async (e) => {
    ...
    if (!ALLOWED_FILE_TYPES.includes(file.type)) {
      setError(
        "Invalid file type. Only PDF, TXT, DOC, JPEG, and PNG files are allowed."
      );
      setShowError(true);
      return;
    }
